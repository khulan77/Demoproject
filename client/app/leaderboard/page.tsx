  ...
  return (
    <div className="min-h-screen bg-[#FDFCFE] relative overflow-hidden pt-20 pb-16">
      <motion.div
        ...
        className="absolute top-[-5%] right-[-5%] w-[400px] h-[400px] bg-purple-100 rounded-full blur-[100px] -z-10"
      />
      <motion.div
        animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.3, 0.2] }}
        transition={{ duration: 10, repeat: Infinity, delay: 1 }}
        className="absolute bottom-[-5%] left-[-5%] w-[400px] h-[400px] bg-green-100 rounded-full blur-[100px] -z-10"
      />
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-6">
          <Link href="/">
            <motion.button
              whileHover={{ x: -5 }}
              whileTap={{ scale: 0.95 }}
              className="hidden items-center gap-2 bg-white px-5 py-2.5 rounded-2xl shadow-[0_4px_0_0_#f3f4f6] active:shadow-none active:translate-y-1 border border-gray-100 text-[#5D3191] font-black text-[10px] uppercase tracking-widest transition-all"
            >
              <ArrowLeft className="w-4 h-4" /> Буцах
            </motion.button>
          </Link>
          <div className="text-center">
            <h1 className="text-[#5D3191] font-[1000] text-3xl md:text-4xl pt-10 pl-30 tracking-tighter gap-3 flex uppercase italic">
              Шилдэг <span className="text-[#8DC63F]">Баатрууд</span>
            </h1>
          </div>
          <div className="w-24 hidden md:block" /> {/* Balance spacer */}
        </div>

        {/* TOP 3 - NEW COMPACT PODIUM */}
